...
percentage_min_instances = float(sys.argv[6])
min_instances_per_leafnode = percentage_min_instances * len(instances)
node_is_leaf = float(sys.argv[7])
start_time = time.time()
utils.nsga(generations, pop_size, max_nodes, features, algorithms, instances, crossover_prob, mutation_prob, min_instances_per_leafnode, node_is_leaf)
end_time = time.time()
